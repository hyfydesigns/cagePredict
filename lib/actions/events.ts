 ...
 */
/**
 * Fetch stats for a specific set of fight IDs (pass all visible fight IDs,
 * not just one event, to avoid missing prelim/co-event fights that may be
 * stored under a different event_id in the DB).
 */
export async function getPicksStats(fightIds: string[], userId: string): Promise<EventStats> {
  if (!fightIds.length) {
  ...
  // 1. Fight details for the supplied IDs
  ...
  // 2. User's predictions for those fights
  ...
  for (const pred of (preds ?? []) as any[]) {
    ...
    if (!fight) continue
    if (pred.is_correct === null) continue   // not yet scored
...
/** @deprecated use getPicksStats with explicit fightIds */
export async function getEventStats(eventId: string, userId: string): Promise<EventStats> {
  const supabase = await createClient()
  const { data: fights } = await supabase
    .from('fights').select('id').eq('event_id', eventId)
  return getPicksStats((fights ?? []).map((f: any) => f.id), userId)
}
